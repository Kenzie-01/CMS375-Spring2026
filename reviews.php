...
// Whitelist role filter to only valid values
...
if ($score_filter > 0) {
    $sql .= " AND Reviews.Score = ?";
    $params[] = $score_filter;
    $types .= 'i';
}
if ($role_filter !== '') {
    $sql .= " AND Users.UserType = ?";
    $params[] = $role_filter;
    $types .= 's';
}
...
<form method="GET" action="reviews.php">
    <div class="filter-bar">
        <select name="score" class="filter-select" onchange="this.form.submit()">
            ...
            <?php for ($i = 10; $i >= 1; $i--): ?>
                <option value="<?php echo $i; ?>" <?php echo ($score_filter == $i) ? 'selected' : ''; ?>><?php echo $i; ?>/10</option>
            <?php endfor; ?>
        </select>
                ...
                <div class="review-top">
                    <div>
                        ...
                        <div class="review-movie" style="margin-top:5px;">
                            on <a href="movie.php?id=<?php echo urlencode($review['MovieID']); ?>"><?php echo htmlspecialchars($review['MovieTitle']); ?></a>
