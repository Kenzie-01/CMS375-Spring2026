...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($first_name) || empty($last_name) || empty($email) || empty($username) || empty($password)) {
        ...
        if (!$error) {
            ...
            $stmt = $conn->prepare("INSERT INTO Users (UserID, FirstName, LastName, Email, UserType, FavoriteGenre, Password) VALUES (?, ?, ?, ?, 'Regular', ?, ?)");
            $stmt->bind_param("ssssss", $username, $first_name, $last_name, $email, $fav_genre, $hashed);
            if ($stmt->execute()) {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .register-container { padding: 50px 20px 60px; display: flex; justify-content: center; }
        .register-box { width: 100%; max-width: 480px; text-align: center; }
        .register-box h1 { font-size: 32px; font-weight: 800; letter-spacing: 4px; margin-bottom: 6px; }
        ...
        .form-group input, .form-group select { width: 100%; padding: 11px 16px; background: #111; border: 1px solid #333; border-radius: 8px; color: #fff; font-size: 14px; transition: border-color 0.2s; }
        .form-group select option { background: #111; }
        .form-group input:focus, .form-group select:focus { outline: none; border-color: #5b80a8; }
        .form-group input::placeholder { color: #444; }
        .error-msg { background: rgba(255,80,80,0.1); border: 1px solid rgba(255,80,80,0.3); color: #ff5050; padding: 10px 16px; border-radius: 8px; font-size: 13px; margin-bottom: 16px; text-align: left; }
        .register-btn { width: 100%; padding: 12px; background: #5b80a8; color: #fff; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; letter-spacing: 1px; cursor: pointer; margin-top: 8px; transition: background 0.2s, transform 0.15s; }
        .register-btn:hover { background: #4a6a90; transform: translateY(-2px); }
        .login-link { display: inline-block; margin-top: 20px; color: #555; text-decoration: none; font-size: 13px; }
        ...
        <form method="POST" action="register.php">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="first_name" placeholder="Jane" value="<?php echo isset($_POST['first_name']) ? htmlspecialchars($_POST['first_name']) : ''; ?>" required>
                </div>
                <div class="form-group">
                    <label>Last Name</label>
                    <input type="text" name="last_name" placeholder="Doe" value="<?php echo isset($_POST['last_name']) ? htmlspecialchars($_POST['last_name']) : ''; ?>" required>
                </div>
            </div>
            <div class="form-group">
                <label>Email</label>
            ...
            <div class="form-group">
                ...
                <input type="text" name="username" placeholder="jane_doe" value="<?php echo isset($_POST['username']) ? htmlspecialchars($_POST['username']) : ''; ?>" maxlength="20" required>
            ...
            <div class="form-group">
                ...
                <select name="fav_genre" required>
                    ...
                    <?php foreach ($genres as $g): ?>
                        <option value="<?php echo $g; ?>" <?php echo (isset($_POST['fav_genre']) && $_POST['fav_genre'] == $g) ? 'selected' : ''; ?>><?php echo $g; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-row">
